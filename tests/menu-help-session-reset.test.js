...
(async () => {
  ...
  console.log('\n── MENU clears an active assessment-session capture ──');
  {
    ...
    check(countSessions(phoneHash) === 0, 'M-02: all session rows cleared after MENU (0 remain)');
    ...
    check(sentMessages[0].text.includes("Here's what I can do"), 'M-05: reply is the help menu content');
  }
  ...
  console.log('\n── HELP clears an active data-assessment ("Upload marks") flow ──');
  {
    ...
    check(countSessions(phoneHash) === 0, 'H-02: all session rows cleared after HELP (0 remain)');
  ...
  console.log('\n── HI and HELLO (aliases of the same branch) also clear sessions ──');
  {
    ...
    check(countSessions(phoneHash) === 0, 'A-01: HI clears session state');

    assessmentSessionState.set(phoneHash, { step: 'ACTIVE', learnerIndex: 3, lastActivity: Date.now() });
    await handleCommand(phone, 'HELLO');
    check(countSessions(phoneHash) === 0, 'A-02: HELLO clears session state');
  }
