...
| [ADR-007](./ADR-007-progress-mastery-coverage-services.md) | Proposed; not yet implemented | ADR-003, ADR-004, ADR-005, `learnerTimelineService` (ADR-003 PR3) | Progress/Coverage/Mastery service layer — scopes `ProgressService` to percentage-bearing assessment events grouped per `(learnerId, subject)`, scopes `CoverageService` to CAPS topic comparison, and defines `MasteryService` as a pure composition of both plus `TimelineService` with no database access of its own; fixes the filter/group contract before PR4 is written. |
